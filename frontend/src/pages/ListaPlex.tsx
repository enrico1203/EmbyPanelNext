...
type SortKey = "pmail" | "reseller" | "server" | "nschermi" | "days_left" | "nota";
...
function sortUsers(users: PlexUser[], key: SortKey, dir: SortDir): PlexUser[] {
...
export default function ListaPlex() {
  const { user } = useAuth();
  ...
  const filtered = users.filter((u) =>
    (u.pmail ?? "").toLowerCase().includes(q) ||
  ...
  return (
    <div className="pg">
      <div className="pg-title">
        Lista Utenti
        <span className="pg-badge platform-plex" style={{ marginLeft: 10 }}>Plex</span>
      ...
      {!loading && !error && users.length > 0 && (
        <div className="page-toolbar">
          <label className="search-field" htmlFor="plex-search">
            <Search size={16} />
            <input
              id="plex-search"
              type="text"
              placeholder="Cerca email, reseller, server..."
              value={search}
      ...
      {loading ? (
        <ListSkeleton columns={isAdmin ? 6 : 5} />
      ) : error ? (
        ...
        <div className="table-card">
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <Th col="pmail">Email Plex</Th>
                  <Th col="days_left">Giorni rimanenti</Th>
                  {isAdmin && <Th col="reseller">Reseller</Th>}
                  <Th col="server">Server</Th>
                  <Th col="nschermi">Schermi</Th>
                  <Th col="nota">Nota</Th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((u) => (
                  <tr key={u.invito} onClick={() => navigate(`/lista/plex/${u.invito}`)} style={{ cursor: "pointer" }}>
                    <td style={{ fontWeight: 600 }}>{u.pmail ?? "—"}</td>
