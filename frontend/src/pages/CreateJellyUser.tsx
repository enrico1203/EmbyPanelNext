...
export default function CreateJellyUser() {
  ...
  const monthlyPrice = useMemo(() => options?.prices?.jellyfin?.[form.screens] ?? 0, [form.screens, options]);
  const cost = useMemo(() => {
    if (!options) return 0;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setSuccess(null);
    try {
      const response = await api.post("/provisioning/jelly", {
      ...
      <div className="manage-header">
        <div>
          <div className="pg-title" style={{ marginBottom: "0.35rem" }}>
            <Tv2 size={18} />
            <span>Crea Utente Jellyfin</span>
          </div>
          <p className="manage-subtitle">
            L'utente viene assegnato al reseller loggato e creato nel server Jellyfin meno usato.
          </p>
        </div>
      </div>
        ...
        <div className="create-grid">
          <section className="config-section">
            <div className="config-section-header">
              <div>
                <h2>Dati account</h2>
                <p>Il costo viene calcolato in base ai giorni e al numero di schermi scelti.</p>
              </div>
            </div>
