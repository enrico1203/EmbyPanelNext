...
function Row({ label, value, accent }: { label: string; value: React.ReactNode; accent?: boolean }) {
  return (
    ...
    >
      ...
      <span style={{ color: accent ? "var(--gold)" : "var(--txt)", fontWeight: accent ? 700 : 500, wordBreak: "break-all" }}>
        {value ?? "—"}
      </span>
    </div>
  );
...
export default function EmbyUserDetail() {
  ...
  const [u, setU] = useState<EmbyUserDetail | null>(null);
  ...
  return (
    <div className="pg">
      ...
      <div style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 18, overflow: "hidden", marginBottom: 16 }}>
        ...
        <Row label="Reseller" value={isAdmin ? u.reseller : undefined} />
      ...
      <style>{`
        @media (max-width: 640px) {
          .detail-row-grid { grid-template-columns: 1fr !important; gap: 2px !important; }
        }
      `}</style>
    </div>
  );
