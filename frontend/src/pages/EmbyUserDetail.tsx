...
interface EmbyActionResponse {
  message: string;
  user: EmbyUserDetailData;
  cost?: number | null;
...
function Row({ label, value, accent }: { label: string; value: React.ReactNode; accent?: boolean }) {
  return (
    <div className="detail-row">
      ...
      <span className="detail-row-value" style={{ color: accent ? "var(--gold)" : "var(--txt)", fontWeight: accent ? 700 : 500 }}>
        {value ?? "—"}
      </span>
    </div>
  );
...
export default function EmbyUserDetail() {
  const { invito } = useParams<{ invito: string }>();
  const navigate = useNavigate();
  const { user: me, refreshUser } = useAuth();
  const isAdmin = me?.ruolo === "admin";

  const [u, setU] = useState<EmbyUserDetailData | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err: any) => {
        if (!active) return;
        setError(err?.response?.data?.detail ?? err.message ?? "Errore durante il caricamento dell'utente Emby.");
      })
      .finally(() => {
  ...
  const pendingMonthlyPrice = Number(options?.prices?.[serviceKey]?.[String(u?.schermi ?? 1)] ?? 0);
  const pendingDays = Math.max(0, u?.days_left ?? 0);
  const pendingCost = calcCost(pendingMonthlyPrice, pendingDays);

  const runAction = async (
    actionKey: string,
    request: Promise<{ data: EmbyActionResponse }>,
    successMessage?: (response: EmbyActionResponse) => string,
  ) => {
  ...
  const handleRenew = async (event: React.FormEvent) => {
    ...
    await runAction(
      "renew",
      api.post(`/users/emby/${invito}/renew`, {
  ...
  const handleDelete = async () => {
    ...
    setNotice(null);
    try {
      const response = await api.delete(`/users/emby/${invito}`);
      setModal(null);
      ...
      setNotice({
        type: "error",
        text: err?.response?.data?.detail ?? "Errore durante la cancellazione dell'utente Emby.",
      });
    } finally {
      setSubmitting(null);
    }
  ...
  const handlePassword = async (event: React.FormEvent) => {
    ...
    await runAction(
      "password",
      api.post(`/users/emby/${invito}/password`, { password: newPassword }),
    );
  };
  ...
  const handleNote = async (event: React.FormEvent) => {
    ...
    await runAction(
      "note",
      api.post(`/users/emby/${invito}/note`, { nota: noteValue }),
    );
  };
  ...
  const handleToggle4k = async (enabled: boolean) => {
    ...
    await runAction(
      action,
      api.post(`/users/emby/${invito}/${action}`),
    );
  };
  ...
  return (
    <div className="pg">
      <button onClick={() => navigate("/lista/emby")} style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: ".8rem", fontWeight: 600, color: "var(--txt-muted)", background: "none", border: "none", cursor: "pointer", padding: "5px 0", marginBottom: 8 }}>
        <ArrowLeft size={14} /> Lista Emby
      </button>
      ...
      <div style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 18, overflow: "hidden", marginBottom: 16 }}>
        ...
        <Row label="Tipo server" value={u.server_type} />
              ...
              <>
                <div className="modal-body">
                  <p style={{ margin: 0, color: "var(--txt)" }}>
                    Vuoi cancellare definitivamente l'utente <strong>{u.user}</strong> da Emby e dal database locale?
                  </p>
