...
export default function ListaJelly() {
  const { user } = useAuth();
  const isAdmin = user?.ruolo === "admin";
  const navigate = useNavigate();
  const [users, setUsers] = useState<JellyUser[]>([]);
  ...
  return (
    <div className="pg">
      <div className="pg-title">
        Lista Utenti
        <span className="pg-badge platform-jelly" style={{ marginLeft: 10 }}>Jellyfin</span>
      ...
      {!loading && !error && users.length > 0 && (
        <div className="page-toolbar">
          <label className="search-field" htmlFor="jelly-search">
            <Search size={16} />
            <input
              id="jelly-search"
              type="text"
        ...
        <div className="table-card">
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {sorted.map((u) => (
                  <tr key={u.invito} onClick={() => navigate(`/lista/jelly/${u.invito}`)} style={{ cursor: "pointer" }}>
                    ...
                    <td>
                      {u.k4?.toLowerCase() === "true"
                        ? <span style={{ color: "#00a4dc", fontWeight: 600 }}>✓</span>
