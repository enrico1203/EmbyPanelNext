...
def _load_users() -> list[dict]:
    return run_select(
        """
        SELECT invito, "user", server, date, expiry
        FROM public.euser
...
def _get_disabled_state(server_name: str, user_id: str) -> bool | None:
    try:
        policy = embyapi.get_user_policy(server_name, user_id)
    ...
    try:
        info = embyapi.get_user_info(server_name, user_id)
...
def run() -> None:
    ...
    if not expired_rows:
        print("bloccautentiemby: nessun utente scaduto da bloccare.")
        return
    ...
    print(f"bloccautentiemby: utenti scaduti trovati {len(expired_rows)}")
    ...
    for row in expired_rows:
        ...
        try:
            user_id = embyapi.get_user_id(server_name, username)
            if not user_id:
            ...
            if disabled_state is True:
                already_disabled += 1
                print(f"bloccautentiemby: {username} su {server_name} gia bloccato")
                continue

            embyapi.disable_user(server_name, user_id)
            blocked_now += 1

            message = f"bloccautentiemby: utente {username} bloccato sul server {server_name}"
            print(message)
            send_telegram_message(message)
        except Exception as exc:
            error_message = f"bloccautentiemby: errore su {username}@{server_name}: {type(exc).__name__}: {exc}"
            errors.append(error_message)
            print(error_message)
            send_telegram_message(error_message)

    summary = (
        f"bloccautentiemby completato. Scaduti: {len(expired_rows)} | "
